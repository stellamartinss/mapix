      ...
      <div className='max-w-2xl w-full bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8'>
        ...
        <div className='mb-8'>
          <h3 className='text-lg font-semibold text-gray-900 dark:text-white mb-4'>
            {t('finalResults_players_capital')} ({players.length})
          </h3>
        ...
        <div className='space-y-3'>
          {isCreator ? (
            <button
              ...
              className='w-full px-4 py-3 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 dark:bg-green-500 dark:hover:bg-green-600 text-white font-medium rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors'
            >
              {loading && <RefreshCcw className='w-4 h-4 animate-spin' />}
              <span>
                {loading ? t('buttons_initiating') : t('buttons_startGame')}
              </span>
            </button>
          ) : (
            <div className='text-center py-3 text-gray-600 dark:text-gray-400'>
              {t('waitingForCreatorToStart')}
            </div>
          )}
