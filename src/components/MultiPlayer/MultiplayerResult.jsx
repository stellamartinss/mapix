...
const MultiplayerResult = ({
  ...
  const currentPlayerRank =
    rankedPlayers.findIndex((p) => p.id === playerId) + 1;
      ...
      <div className='bg-gray-800 border-b border-gray-700 p-4 flex items-center justify-between flex-shrink-0'>
        <div className='flex items-center gap-4'>
          <h1 className='text-2xl font-bold text-white'>
            🎉 {t('finalResults_title')}
          </h1>
          <code className='text-lg font-mono text-indigo-400'>{room.code}</code>
        </div>
            ...
            <div className='flex items-center gap-2'>
              ...
              <div>
                <p className='text-xs text-gray-400'>
                  {t('finalResults_yourPosition')}
                </p>
                <p className='text-xl font-bold text-white'>
                  {currentPlayerRank}
                  {t('finalResults_positionSuffix')}
                </p>
              </div>
            </div>
              ...
              <div className='border-t border-gray-700 pt-2 space-y-1'>
                {rankedPlayers.slice(0, 5).map((player) => (
                  <div key={player.id} className='flex items-center gap-2'>
                    ...
                    <span
                      className={`text-xs truncate ${player.id === playerId ? 'text-indigo-300 font-semibold' : 'text-gray-400'}`}
                    >
                      {player.name}
                    </span>
                  </div>
                ...
                {rankedPlayers.length > 5 && (
                  <p className='text-xs text-gray-500 italic'>
                    +{rankedPlayers.length - 5} {t('finalResults_players')}
                  </p>
                )}
              </div>
            </div>
          ...
          <div className='max-w-2xl mx-auto w-full'>
            ...
            <div className='mt-6 bg-gray-800 rounded-lg p-4'>
              <p className='text-sm text-gray-400 text-center'>
                {t('finalResults_realLocation')}: {room.location.lat.toFixed(6)}
                , {room.location.lng.toFixed(6)}
              </p>
            </div>
          </div>
        </div>
